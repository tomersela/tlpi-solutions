...
```
$ ./flock_inheritance
TEST: Duplicated descriptors refer to same lock:
   SUCCESS: Lock released via duplicated descriptor

TEST: Multiple open() calls create independent locks:
   SUCCESS: Process locked itself out

TEST: Child can release parent's lock:
   SUCCESS: Child released parent's lock

TEST: Lock released only when ALL duplicates closed:
   SUCCESS: Lock persists after closing original fd
   SUCCESS: Lock released after closing all duplicates
```

Section 55.2.1 semantics verified!

...
static void
handler(int sig, siginfo_t *si, void *uc) // 1
{
    timer_t *tidptr;
...
main(int argc, char *argv[])
{
    ...
    for (j = 0; j < argc - 1; j++) {
        itimerspecFromStr(argv[j + 1], &ts); // 3

        sev.sigev_value.sival_ptr = &tidlist[j];
                /* Allows handler to get ID of this timer */

        if (timer_create(CLOCK_REALTIME, &sev, &tidlist[j]) == -1) // 4
            errExit("timer_create");
        printf("Timer ID: %ld (%s)\n", (long) tidlist[j], argv[j + 1]);

        if (timer_settime(tidlist[j], 0, &ts, NULL) == -1) // 5

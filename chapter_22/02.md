In Linux, standard signals get precedence over realtime signals.
...
## Test run
We see that the standard signal takes precedence regardless of the order of sending the signals:

```
> ./sigorder
Sending SIGUSR1
Sending realtime signal - 42
caught signal 10
 si_signo=10, si_code=-6 (other), si_value=0
si_pid=6755, si_uid=1000
caught signal 42
 si_signo=42, si_code=-1 (SI_QUEUE), si_value=0
si_pid=6755, si_uid=1000
>
> ./sigorder rt
Sending realtime signal - 42

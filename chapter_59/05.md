...
```
$ ./udp_connect_test
sockA bound to 127.0.0.1:12345
sockB bound to 127.0.0.1:12346
sockC bound to 127.0.0.1:12347
sockA connected to sockB
sockC sending to sockA address...
sendto() from C to A succeeded
ERROR [EAGAIN/EWOULDBLOCK Resource temporarily unavailable] recvfrom on sockA
sockA did not receive message
sockA sending to sockB...
send() from A to B succeeded
sockB received: 'Hello from sockA'
```

## Conclusion

UDP `connect()` filters incoming packets to only accept from the connected peer. Socket A rejected the packet from socket C because it was connected to socket B.

This behavior is described in TLPI under section 56.6.2 "Using connect() with Datagram Sockets".
